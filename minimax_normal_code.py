...
def Minimax(array, pc_f, user_f):
    ...
    for i in range(4):
        for j in range(4):
            if array[i][j] == '':
                array[i][j] = pc_f
                score = Minimize(array, pc_f, user_f, nodes_explored, current_depth + 1, max_depth)
                if score > v:
                    coordonne = (i, j)
                    v = score
                array[i][j] = ''
    ...
    print("depth: " + str(max_depth[0]))
    
    if coordonne:
        array[coordonne[0]][coordonne[1]] = pc_f
    return array

def Minimize(array, pc_f, user_f, nodes_explored, current_depth, max_depth):
    ...
    for i in range(4):
        for j in range(4):
            if array[i][j] == '':
                array[i][j] = user_f
                score = Maximize(array, pc_f, user_f, nodes_explored, current_depth + 1, max_depth)
                v = min(v, score)
                array[i][j] = ''
    return v

def Maximize(array, pc_f, user_f, nodes_explored, current_depth, max_depth):
    var = GuessWinner(array, pc_f, user_f)
    if var is not None:
        return var
